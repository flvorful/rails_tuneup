module RailsTuneup::TuneupHelper #:nodoc:

  include RailsTuneup::BumpsparkHelper

  def tuneup_signin_url
    "#{RailsTuneup.collector_url}/users"
  end

  def tuneup_signup_url
    "#{RailsTuneup.frontend_url}/signup"
  end
  ...
  def tuneup_collecting?
    RailsTuneup.collecting
  end

  def tuneup_data
    most_recent_data = RailsTuneup.stack.first
    most_recent_data.blank? ? most_recent_data : most_recent_data['stack']
  end

  def tuneup_schemas
    RailsTuneup.stack.first['schemas']
  end

  def trend
    numbers= if RailsTuneup.trend.size > 50
      RailsTuneup.trend[-50..-1]
    else
      RailsTuneup.trend
    end
  ...
  def tuneup_bar(step=tuneup_data, options={})
    ...
    bars = RailsTuneup::Step.layers.map do |layer|
  ...
  def tuneup_open_run(token)
    %[window.open("#{RailsTuneup.frontend_url}/runs/#{token}", 'fiveruns_tuneup');]
  end
