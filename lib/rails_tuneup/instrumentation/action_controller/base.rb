      ...
      module ActionController
        module Base
          def self.included(base)
            RailsTuneup.instrument base, ClassMethods, InstanceMethods
          end
          module ClassMethods
            def cache_page_with_fiveruns_tuneup(*args, &block)
              RailsTuneup.step "Cache page", :controller do
                cache_page_without_fiveruns_tuneup(*args, &block)
              end
            end
            def expire_page_with_fiveruns_tuneup(*args, &block)
              RailsTuneup.step "Expire cached page", :controller do
          ...
          module InstanceMethods
            def perform_action_with_fiveruns_tuneup(*args, &block)
              RailsTuneup.run(self, request) do
                action = (request.parameters['action'] || 'index').to_s
                if RailsTuneup.recording?
                  RailsTuneup.instrument_filters(self) 
                  RailsTuneup.instrument_action_methods(self) 
                  RailsTuneup.instrument_custom_methods
                end
                result = RailsTuneup.step "Perform #{action.capitalize} action in #{self.class.name}", :controller, false do
