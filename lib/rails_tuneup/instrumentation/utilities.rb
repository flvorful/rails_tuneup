    ...
    module Utilities
      ...
      def step(name, layer=nil, link=true, sql=nil, table_name=nil, &block)
        if recording?
          ...
          returning ::RailsTuneup::Step.new(name, layer, file, line, sql, &block) do |step|
      ...
      def wrap(klass, format, meth, name, layer)
        ...
        def #{format % :with}(*args, &block)
          RailsTuneup.step "#{name}", :#{layer} do
      ...
      def install_instrumentation
        ...
        Dir[File.join(instrumentation_path, '/*/**/*.rb')].each do |filename|
          ...
          instrumentation = "RailsTuneup::Instrumentation::#{constant_name}".constantize
